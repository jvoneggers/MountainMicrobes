...
```{r}
ESV_names<-names(esv_table) keep<-read.csv("wanted_samples.csv",header=F) %>% pull(.,var=V1)
positions_keep<-NULL 
...
remove_df<-esv_table[,-positions_keep] (str_split(names(remove_df),pattern = ".16S.", 2, simplify = T)[,1])
