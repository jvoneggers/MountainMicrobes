...
```{r}
# read in fasta file 
fasta <- readDNAStringSet("../1_RawSequenceProcessing/output/OTU_97_no_chimeras_denovo.fa")

# read in assigned taxonomy 
tax_table<-read.csv("../1_RawSequenceProcessing/output/OTU_97_taxonomy_80_DADA2.csv", header=T)
...
tax_table$ESV<-NULL
write.csv(tax_table,"Taxonomy/OTU_97_Silva138_assignedTaxonomy_toSpecies.csv")
rm(ESVseq);rm(fasta)
...
```{r}
tax_tab<-read.csv("Taxonomy/OTU_97_Silva138_assignedTaxonomy_toSpecies.csv", header=T,row.names=1)
...
## 2. Read in ESV table
```{r}
esv_table<-read.delim("../1_RawSequenceProcessing/output/OTU_table_97", header = TRUE, stringsAsFactors = FALSE, quote = "", sep = "\t")
...
if(length(which(reads_per_esv$reads<1))>0){ esv_table<-esv_table[which(reads_per_esv$reads>0),]
}
...
plot(table_reads_per_esv$reads,table_reads_per_esv$freqESVs,main="Number of ESVs per read count", xlab="read count", ylab="number of esvs")

#plot the subset of 0-20 reads  
plot(table_reads_per_esv$reads[1:100],table_reads_per_esv$freqESVs[1:100],main="number of esvs per read count", xlab="reads", ylab="number of esvs")
...
Citation for removing OTUs with less than reads 10: 
...
```{r}
#write.csv(esv_table,file=paste0("OTU/",Sys.Date(),"_OTU_97_table_notnorm.csv"),row.names = T)
write.csv(tax_tab,file=paste0("OTU/",Sys.Date(),"_OTU_97_tax_tab_notnorm.csv"),row.names = T)
```
...
#write.csv(esv_to_normalize_10k, paste0("OTU/",Sys.Date(),"_OTU_97_ESVs_to_normalize_10K_reads.csv"), row.names=T)
write.csv(esv_tab_10knorm, paste0("OTU/",Sys.Date(),"_OTU_97_ESV_tab_10K_reads_normalized.csv"), row.names=T)
